...
class ProductController extends Controller
{
    ...
    {
        $query = Product::query()
            ->with(['brand:id,name', 'category:id,name', 'currency:code,symbol', 'images']);
        ...
        $products->getCollection()->transform(function ($product) {
            $product->image_url = $product->image_main 
                ? Storage::url($product->image_main) 
                : null;
            return $product;
        });
    ...
    public function store(ProductRequest $request): RedirectResponse
    {
        ...
        // Le tenant_id sera automatiquement ajouté via le trait BelongsToTenant
